...
```{.error}
Error in library(S4Vectors): there is no package called 'S4Vectors'
```
...
```{.error}
Error in DataFrame(Integers = c(1L, 2L, 3L), Letters = c("A", "B", "C"), : could not find function "DataFrame"
```
...
```{.error}
Error in as.data.frame(DF1): object 'DF1' not found
```
...
```{.error}
Error in .class1(object): object 'df1' not found
```
...
```{.error}
Error in rownames(DF1): object 'DF1' not found
```
...
```{.error}
Error in as.data.frame(DF3): object 'DF3' not found
```
...
```{.error}
Error in mcols(DF4): could not find function "mcols"
```
...
```{.error}
Error in DataFrame(Type = sapply(DF4, typeof), Distinct = sapply(DF4, : could not find function "DataFrame"
```
...
```{.error}
Error in Rle(v1): could not find function "Rle"
```
...
```{.error}
Error in eval(expr, envir, enclos): object 'rle1' not found
```
...
```{.error}
Error in Rle(v2): could not find function "Rle"
```
...
```{.error}
Error in DataFrame(vector = v2, rle = rle2, equal = v2 == rle2): could not find function "DataFrame"
```
